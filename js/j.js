$(document).ready(function () {
    ...
    $priceLine.on("mousemove", function (e) {
        ...
        if ($dragging && ($dragging.hasClass('price-from')) && (nV >= 0) && (nV < (rightLimit - $dragging.width()))) {
        ...
        if ($dragging && ($dragging.hasClass('price-to')) && (nV <= (this.offsetWidth - $dragging.width() - 1)) && (nV > (leftLimit + $dragging.width()))) {
            $dragging.css('left', nV);
            $pTI.val(Math.round(nV / (this.offsetWidth - $dragging.width() - 1) * 24000));
            $priceLineFill.css({
